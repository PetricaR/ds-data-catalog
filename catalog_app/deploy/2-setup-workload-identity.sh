...
grant_iam_permissions() {
    ...
    local roles=(
        "roles/aiplatform.user"              # Vertex AI / Gemini Insights
        "roles/bigquery.jobUser"             # Run BQ preview / quality jobs
        "roles/bigquery.dataViewer"          # Read BQ table data
        "roles/bigquery.metadataViewer"      # Read BQ schema / metadata
        "roles/secretmanager.secretAccessor" # Read BQ SA key from Secret Manager
        "roles/logging.logWriter"            # Write Cloud Logging
        "roles/monitoring.metricWriter"      # Write Cloud Monitoring
    )
...
print_summary() {
    ...
    echo -e "${GREEN}✓ BigQuery Access: ${NC}Granted (jobUser, dataViewer, metadataViewer)"
    echo -e "${GREEN}✓ Vertex AI Access: ${NC}Granted (aiplatform.user for Gemini Insights)"
    echo -e "${GREEN}✓ Secret Manager Access: ${NC}Granted (secretAccessor)"
